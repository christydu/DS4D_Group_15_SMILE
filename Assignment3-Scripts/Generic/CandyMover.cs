...
public class CandyMover : MonoBehaviour {
	...
	void checkCorrectPlacement (){
		...
		for(int cnt = 0; cnt < availableCustomers.Length; cnt++) {
			print ("theCustomer: " + availableCustomers[cnt].name);
			if(availableCustomers[cnt].GetComponent<CustomerController>().isCloseEnoughToCandy) {
				...
				print ("theCustomer: " + theCustomer.name);
				delivered = true;
			}
		}
		...
		if(delivered) {
			...
			int currentInventory = PlayerPrefs.GetInt("availableCandy");
			PlayerPrefs.SetInt("availableCandy", --currentInventory);

...
public class CandyController : MonoBehaviour {
	...
	private bool canCreate;					//custom flag to prevent double picking
	private bool canBuyCandy = false;		//are we allowed to buy more candy?
	...
	void Awake (){
		...
		//availableCandy = 5;
		availableCandy = PlayerPrefs.GetInt("availableCandy");
	}
	...
	void updateAvailableCandies (){
		...
		//we no longer need to buy candy from in-game button. We do it in IAP.
		/*
		if(canBuyCandy)
	...
	void createCandy (){
		if(canCreate && !MainGameController.gameIsFinished) {
			...
			playSfx(itemPick);
			canCreate = false;
			IngredientsController.itemIsInHand = true; //prevent user from picking an ingredient when trying to deliver a candy!
